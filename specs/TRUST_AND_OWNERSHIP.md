...
**Authority:** this file. **Constitutional anchors:** Art. XXV ("`/chat` is the only wire"), Art. XXXIV.5 (rappid invariants), MASTER_PLAN Part Deux §3 (no mandatory PKI) and §4 (un-shutdownable). **Normative cross-refs:** `rapp-rappid/2.0` (Eternity identity), `rapp-moment/1.0` §6 (keypair-bound ownership, deed chains), `rapp-protocol/1.0` §2 (the canonical rappid string), the RAPP **compatibility contract** (read-all-legacy / emit-canonical / hash-is-join-key).
...
**The load-bearing fact of this spec:** a rappid is fully-formed with **zero keys**. It is content-addressed where there is content to bind — e.g. `sha256("moment:"+pk)` for a Moment, `sha256("keeper:"+pubx)` for a zookeeper key — and a fresh `sha256(uuid4().bytes)` mint otherwise. **A keypair is never an input to minting an identity.** Identity precedes, and is independent of, any signature.

> Cross-ref: identity format & invariants are governed by `rapp-rappid/2.0` (Eternity). Eternal ids are published in the Eternity hub (`rapp-egg-hub`). This spec does not redefine identity; it defines **authority over** an identity.
...
|------|--------------------|-----------|-----------|
| **L0 — Identity** | "Who/what is this?" | SHA-256 content-address (the rappid) | Always present, PKI-free |
...
| **L2 — Sovereign authority** | "Can ownership be proven without GitHub?" | **Optional keypair** binding (`sig_suite: ecdsa-p256`) | **Opt-in only**; never required |
...
## §4 — L2: optional keypair sovereignty (`sig_suite: ecdsa-p256`)

An owner **MAY** bind a keypair to a rappid for **sovereignty that outlives GitHub** — survive-takedown, survive-account-loss, survive-death. This is the same keypair model the `vNeighborhood` twins and `rapp-moment` zookeepers already mint (ECDSA P-256 via Web Crypto, public JWK published, private key held client-side).

- A binding is a record `{rappid, sig_suite: "ecdsa-p256", pub: <JWK>, ...}` committed to the home repo (e.g. `rappid.json#owner_key` or `lineage/`), establishing that *this public key speaks for this rappid*.
...
**Rule L2.c — verifiable-if-present.** *When a signature IS present, a verifier MUST verify it against the bound key and MUST reject the action if verification fails. An invalid signature is a hard failure (it is a forgery claim); a missing signature is not.*
...
1. **`sig_suite` enum is frozen at `{ none, ecdsa-p256 }`.** `none` selects the L1 branch and means there is no `sig`/`pub`. `ecdsa-p256` selects the L2 branch. New suites are added by **enum extension** (a schema bump, never a string re-version), per the compatibility contract. `sig_suite` is the crypto-agility field and, when a signature is present, is **covered by** that signature.
2. **`canonicalBody(P)`** = the record with keys sorted lexicographically, **excluding** `sig`, `pub`, and any `_`-prefixed key (identical to `rapp-moment/1.0` §6). Signature is hex `r‖s` over the SHA-256 of the UTF-8 canonical body.
...
4. **Invalid signature ≠ missing signature.** `P.sig` present but failing verification → **deny** (forgery). `P.sig` absent → **do not deny**; evaluate L1.
...
`rapp-moment/1.0` §6.1 states a Moment's eternal address maps to `rappid:<slug>:sha256("moment:"+pk)` and a zookeeper to `rappid:keeper:sha256("keeper:"+pubx)`. Read naively this looks like it **mandates** a keypair. It does not. To de-conflict canon explicitly:

- **`sha256("moment:"+pk)` is the OPTIONAL, keypair-bound PROFILE of ownership**, used when an owner has chosen sovereignty (a zookeeper key). The hash binds the *content* (`pk`, the spacetime coordinate); the *key* enters only at the ownership/transfer layer (§6.2 deeds).
...
- Therefore `rapp-moment/1.0` is **a profile of `rapp-trust/1.0`, not an exception to it.** "To own a moment, sign it" is the *sovereign* path (L2); "commit a moment as a collaborator" is the *default* path (L1). Both mint the same eternal, PKI-free identity.

**Rule §7.** *Any spec that derives an id from a public key MUST treat the key as the optional-sovereignty path and MUST define a keyless, gh-collaborator default. A key-derived id MUST NOT be the only way to mint or own.*
...
1. **Read all legacy forms forever.** Verifiers MUST accept every historical authority signal: bare unsigned repo commits, `actors` allowlists, merged PRs, `ecdsa-p256` signatures, and legacy `sig` shapes (a `pk`, a bare UTC ms, a `|`-separated body). A record with **no `sig_suite`** is read as `sig_suite: none`.
2. **Emit only canonical.** New records MUST write the canonical `sig_suite` enum and the canonical rappid string. Never re-version the identity string; **add record fields** for new capability.
...
- **Leviathan / fleet async (Decision #2).** Fleet messaging is **signed twin-chat events over `/chat`** (per `rapp-commons-event/1.0` + `rapp-resident`), authorized by §5 — **not** the unauthenticated `/api/agent` route. A fleet event from a sovereign twin uses the L2 branch (its `ecdsa-p256` signature); an event from a same-operator twin can use L1. This is exactly why the un-authenticated direct route is being closed: `rapp-trust` gives the fleet a *first-class* authorization path on the sacred wire, so the RCE bypass is unnecessary.
...
```jsonc
{
  ...
  "sig_suite": "ecdsa-p256",          // {none|ecdsa-p256}; absent ⇒ "none"
...
**Setup.** Kody plants the door `rappid:@kody-w/wildhaven:9f1c…<64hex>`. He does **not** mint a key. `rappter1` wants to act as the Wildhaven gate twin.
...
4. Kody decides Wildhaven must survive him. In the browser he mints an ECDSA P-256 keypair, publishes the public JWK to `rappid.json#owner_key`, sets `sig_suite: "ecdsa-p256"`, commits.
...
8. An attacker sends an event claiming to be Wildhaven with a bogus `sig`. §5 clause 4: `P.sig` present but `verify` false → **deny** (hard failure). A *missing* sig from a non-collaborator would simply fall through L1 to deny by absence-of-grant — a different, softer path.

**The lesson the corpus must encode:** Wildhaven was a real, owned, tradeable identity on Day 1 with **no cryptography**. The key was an *upgrade Kody chose*, and that choice is what let Wildhaven outlive GitHub. Optionality is the bridge between "no mandatory PKI" and "un-shutdownable."
...
- [ ] It mints/accepts identities with **zero keys** (L0 PKI-free).
...
- [ ] It **verifies** present signatures and **denies on invalid** ones (Rule L2.c / §5.4).
- [ ] Its verifier implements the **OR** of L1/L2, never the AND (§5.3).
- [ ] It reads `sig_suite`-absent as `none` and accepts all legacy authority signals (§8).
